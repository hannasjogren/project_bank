...
try:
    with engine.connect() as conn:
        conn.execute(text("SELECT 1"))  # Använd text() för att göra det exekverbart
...
def import_customers():
    ...
    except SQLAlchemyError as import_denied_to_db:
        session.rollback()
        logger.error(f"Fel vid import av kunder: {import_denied_to_db}")
    finally:
        session.close()
...
def import_accounts():
    ...
    except SQLAlchemyError as import_acc_denied_to_db:
        session.rollback()
        logger.error(f"Fel vid import av konton: {import_acc_denied_to_db}")
    finally:
        session.close()
...
def import_transactions():
    ...
    except SQLAlchemyError as import_transactions_denied_to_db:
        session.rollback()
        logger.error(f"Fel vid import av transaktioner: {import_transactions_denied_to_db}")
    finally:
        session.close()

if __name__ == "__main__":
    logger.info("Startar pipeline för dataimport")
